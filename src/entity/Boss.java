...
/**
 * 쫄몹 로직과의 합의:
 *  - HP1 = B/C 계열, HP2 = A 계열 재활용 (외부 스폰 훅에서 실제 타입 선택)
 *  - 보스 무적은 쫄몹 생존수 > 0 여부로만 결정
 *  - P1: 시작 시 HP1 5기 → 3갈래
 *  - P2: HP가 5(=50%) 되는 순간 진입, HP2 5기 리스폰 → 5갈래
 *  - 발사 타이밍은 프레임 카운터 기반(간단/결정론)
 */
public class Boss {
    ...
    public Boss(
        ...
        // 시작: P1 방패(HP1) 5기
        if (spawnHP1Group != null) spawnHP1Group.run();
    ...
    public void update() {
        // 이동
        this.y += (phase == BossPhase.P1 ? speedP1_pxPerFrame : speedP2_pxPerFrame);
    ...
    private void fireNWay(int ways, int dx) {
        ...
        for (int i = 0; i < ways; i++) {
            int offset = (i - mid) * dx;
            emitter.fire(this.x + offset, spawnY, 0, bulletVy);
        }
    }
...
// ===== 같은 파일의 package-private 타입들 =====
enum BossPhase { P1, P2 }
